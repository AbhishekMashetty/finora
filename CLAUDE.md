...
- **REST only, for now.** Communication between services is synchronous HTTP/JSON through documented contracts (`architecture/api-contracts.md`). Asynchronous messaging (NATS) is a planned future seam (Phase 7 of `architecture/development-roadmap.md`), not built yet. Do not introduce it early — `plan.md` explicitly says not to add complexity before there's a legitimate reason.
