...
### Phase 7 progress log (✅ complete, 2026-07-31)

- **2026-07-31 — Async messaging seam: done.** User asked to finish Phase 7 (the roadmap's "introduce NATS for domain events, replacing synchronous REST notification calls with published events while keeping REST for queries" goal) and confirm the Docker stack was up. Built the two foundational `shared/` packages first, fully integration-tested in isolation against real disposable infrastructure, before touching any service — the same "prove the building block, then replicate" discipline earlier phases established.
  - **`shared/eventbus`** (new): a thin NATS **JetStream** wrapper (`Connect`, `EnsureStream`, `Publish` with an optional `Msg-Id` for dedup, `Subscribe` against a named durable consumer, an `eventbus.Checker` for `/ready`). JetStream was chosen deliberately over core NATS pub/sub for exactly one reason: at-least-once delivery + message persistence + durable consumers, so a subscriber that's down when an event is published still receives it once it's back — core pub/sub has none of that. Proven via `shared/natstest` (mirrors the existing `shared/mongotest` pattern, a real disposable `nats:2-alpine` testcontainer) and 4 integration tests, the most important being `TestBus_DurableConsumer_ResumesAfterRestart` — a message published before any consumer exists is still delivered to a durable consumer created later, the entire reason JetStream was picked.
  - **`shared/outbox`** (new): a Mongo-backed transactional outbox (`Store.Enqueue`, `Relay.Run`/`RelayOnce`) — a service writes its event to its own `outbox_events` collection in the same call path as the triggering write, and a background relay polls for unpublished events and retries publishing to NATS on failure. **Explicitly documented as not a true atomic outbox**: real atomicity needs a MongoDB replica set (multi-document transactions), and this project's Mongo containers run standalone (no `--replSet` in `docker-compose.yml`, the same local-footprint trade-off `architecture/database-design.md` already accepts) — so `Enqueue` is a second, non-atomic insert, a narrow documented race window, accepted rather than engineered around, mirroring how budget-service's pre-existing overspend race was handled. 4 integration tests, including a full end-to-end proof (`TestOutbox_EndToEnd_WithRealNATS`) that `shared/outbox` and `shared/eventbus` compose correctly together.
  - **Wired identically across the three affected services**, replicating the same producer/consumer pattern each time (domain event constants/structs → `internal/events.OutboxPublisher` where applicable → service-layer logic → `main.go` wiring → `config.go` env vars → tests):
    - **expense-service** (producer only): `transactionService.publishCreated` enqueues `finora.transaction.created` right after every successful transaction insert — both the normal `Create` path and each row of the existing CSV import loop. A publish failure is logged and never fails the transaction write itself (the write already succeeded; failing the caller would wrongly imply "retry," which would create a duplicate transaction) — the same "errors are values, handled at the boundary" fail-open pattern this codebase already uses elsewhere.
    - **budget-service** (both consumer and producer): a new `OverspendService.HandleTransactionCreated` consumes `finora.transaction.created` via a durable consumer (`budget-service-transaction-created`), lists the event's user's budgets, and for each recomputes the **current period's** actual spend via the pre-existing `expenseClient.SumExpensesByCategory` REST call — kept as synchronous REST since it's a genuine query, not a notification (`CLAUDE.md` §2's new "REST for queries, async events for domain notifications" rule). Publishes `finora.budget.overspent` on overspend, using the identical `LastNotifiedAt`-exact-period-match dedup rule Phase 4 established for the old REST trigger, now evaluated once per event instead of once per report read. The entire Phase 4–6 synchronous call to notification-service is deleted: `domain.NotificationClient` and `internal/client/notification_client.go` removed, `NewReportService` back to a 2-arg constructor, `Summary()` a pure read again.
    - **notification-service** (consumer only, no outbox): a new `OverspendConsumer.HandleBudgetOverspent` consumes `finora.budget.overspent` via a durable consumer (`notification-service-budget-overspent`), formats the identical `"Budget exceeded"` / `"You've spent {actual} of your {budgeted} {period} {category} budget."` shape the old REST trigger produced, and calls the existing `notificationService.Create` — zero frontend changes needed.
  - **`docker-compose.yml`**: new `nats` service (`nats:2-alpine`, `-js` JetStream enabled, `-sd /data` explicit storage dir — without it JetStream's default storage location doesn't survive container recreation, silently defeating the whole point of durable consumers the moment the NATS container itself restarts, `-m 8222` monitoring endpoint for the healthcheck), wired as a `depends_on: condition: service_healthy` dependency into expense-service/budget-service/notification-service alongside a new `NATS_URL` env var each. `.env.example` updated with the new `NATS_URL`/`NATS_PORT`/`OUTBOX_RELAY_INTERVAL` section and a corrected note that budget-service no longer calls notification-service via REST.
  - **A real toolchain-pinning incident, same class as prior phases':** `nats.go`'s latest version (v1.52.0) requires Go ≥1.25; this repo is pinned to 1.21.3 everywhere. Bisected in a scratch module (v1.45.0 needs 1.23, v1.39.1 needs 1.22) down to **v1.38.0**, the newest version still compatible with 1.21.3 — verified in isolation before touching the real repo, the lesson Phase 6 already reinforced once for `golang.org/x/time`.
  - **A second, larger dependency-ripple incident:** adding `nats.go` to `shared/go.mod` (for `shared/eventbus`) changed the Go workspace's MVS resolution enough that transitive dependency versions shifted even for services that never touch NATS — `docker compose up --build` failed on the **gateway**'s build with "missing go.sum entry for... golang.org/x/text/language" despite zero source changes to gateway. Root-caused as the identical class of bug Phase 6 hit once already (a `shared/` dependency change rippling into every service's `go.sum`, not just the ones directly modified) and fixed the same way: ran `go mod tidy` in all **five** services, not just the three directly touched. Confirmed via `git status` that gateway's and user-service's `go.mod`/`go.sum` did in fact need changes despite no source edits. After this fix, the full stack rebuilt clean — all 11 containers (10 original + the new `nats`) healthy.
  - **Live end-to-end verification, twice.** First attempt through the real running stack produced **zero** notifications despite an intentionally over-budget transaction — traced systematically rather than guessed at: confirmed expense-service's `outbox_events` had the event enqueued and marked published; confirmed budget-service's durable consumer had received and acked it (via NATS's own JetStream monitoring API, `docker compose exec -T nats wget -qO- http://localhost:8222/jsz?...` — port 8222 isn't published to the host, so this required exec'ing into the container); found budget-service's own `outbox_events` was empty, meaning the overspend condition itself was never true. Root cause: the test transaction had no `category_id` attached, so `SumExpensesByCategory`'s name-based match correctly found zero spend — a test-setup gap, not an implementation bug. Redid the test with a real category created via `POST /api/v1/categories` and attached to the transaction: the full chain worked — `POST /transactions` (amount 75, budget 50/monthly/groceries) → expense-service's outbox → NATS → budget-service's consumer → recomputed actual (75) via the existing REST call → detected overspend → budget-service's outbox → NATS → notification-service's consumer → a real notification (`"Budget exceeded"` / `"You've spent 75.00 of your 50.00 monthly groceries budget."`) visible via `GET /api/v1/notifications`.
  - **Verification: done, comprehensively, in a follow-up pass.** Confirmed the running stack still healthy (all 11 containers). Ran `gofmt -l`/`go vet ./...`/`go build ./...`/`go test ./...` from inside each of the 6 Go modules individually (this repo's Go 1.21 workspace has a known limitation where `./...` must be run per-module, not from the workspace root — see the repo gotchas memory) — all clean, including a deliberate re-verification of **gateway** and **user-service**, the two services whose `go.mod`/`go.sum` were touched only by the emergency `go mod tidy` fix above and hadn't had their own test suites re-run since. Ran `shared/eventbus`'s and `shared/outbox`'s full `-tags=integration` suites (8 tests total) against real Docker containers — all passing.
  - **Documentation** (this pass): `CLAUDE.md` §2 rewritten from "REST only, for now" to describe the NATS exception and the outbox pattern. `architecture/api-contracts.md` gained a full "Async Events" section (stream/subject names, both event payloads, the outbox non-atomicity trade-off, JetStream dedup, current-period-only design rationale) and the old budget-service → notification-service subsection marked superseded. `architecture/database-design.md` gained the new `outbox_events` collections for expense-service and budget-service. `architecture/system-overview.md`'s diagram and Future Evolution table updated to show NATS as built, not future. `architecture/development-roadmap.md`'s Phase 7 entry marked complete with a verification summary, matching Phases 0–6's style. `services/expense-service/README.md`, `services/budget-service/README.md` (the more substantial rewrite — removed every `NotificationClient`/Phase-4-REST reference, added the new event-driven overspend section), and `services/notification-service/README.md` updated with their new async-event behavior, env vars, and architecture-layout entries.
