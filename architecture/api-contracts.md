...
To compute `actual` spend per category, budget-service calls **expense-service** directly over REST, forwarding the caller's `X-User-Id` header exactly as the gateway would (no JWT — this is an internal-network call, same trust model as `shared/middleware.RequireIdentity`):

1. `GET /api/v1/categories` on expense-service, to build an id → name map (a budget's `Category` field is a plain name, e.g. `"groceries"`; transactions are grouped by expense-service's internal `category_id`).
2. `GET /api/v1/transactions/aggregate?from=<>&to=<>&type=expense` on expense-service — a single `$match`+`$group` aggregation query returning every category's total (and transaction count) for the range in one round trip, keyed by `category_id`.

Two calls total, **independent of how many budgets the caller has** or how many transactions exist in range. `report_service.go` makes this pair once per report (every budget in a report already shares one caller-supplied `[from, to]`); `overspend_service.go` makes it once per **distinct budget period** a user's budgets span (weekly/monthly/yearly each have their own period start, so at most 3 calls, typically 1). A budget whose category name has no match among the returned totals (case-insensitively) reports `actual = 0`, **not an error** — the user simply hasn't logged anything under that name yet.

This replaced a design that made one call **per budget**: resolve that budget's category name to an id, then paginate through every matching transaction (capped at 50 pages = 5,000 transactions) summing client-side. A user with 8 budgets could generate 30+ internal HTTP calls and dozens of Mongo queries for a single transaction write — the fleet's worst internal request-amplification problem (see `infrastructure/scale-readiness-review.html`, finding F01). The aggregation endpoint moves that summation into a single indexed `$group` query server-side instead.

Implemented in `services/budget-service/internal/client/expense_client.go` behind `domain.ExpenseClient`, so `internal/service/report_service.go` and `internal/service/overspend_service.go` depend only on that interface (Dependency Inversion), never a concrete HTTP client — this keeps both unit-testable with a fake, per `CLAUDE.md` §7.
...
**Consumer retry, backoff, and dead-lettering.** Every `shared/eventbus.Bus.Subscribe` consumer (`MaxDeliver: 10`, `AckWait: 30s`, `MaxAckPending: 256`) applies its own redelivery backoff rather than relying on the JetStream consumer's `BackOff` config: `nats.go`'s `Msg.Nak()` "does not adhere to AckWait or Backoff configured on the consumer and triggers instant redelivery" (its own doc comment), so a bare `Nak()` on every retryable failure would hot-retry as fast as NATS can redeliver — turning a partial downstream outage into a self-inflicted retry storm against the exact dependency that's already struggling. `Subscribe` instead calls `NakWithDelay` with a growing schedule (1s, 5s, 15s, 30s, 1m, 5m) keyed off the message's own delivery attempt count. A handler error wrapping `eventbus.ErrTerminal` — or a retryable error on the delivery attempt that exhausts `MaxDeliver` — is `Term()`'d (never redelivered again) and published to `finora.dlq.<durable-name>` on the same stream, so a permanently-failing message becomes a durable, observable event instead of one that silently stops existing after a log line scrolls off. Up to 32 messages are handled concurrently per consumer (the underlying NATS client delivers to `Consume`'s callback one at a time, so without this a slow handler would pin a consumer's whole throughput to `1/handler_latency`).
...
Consumed by budget-service's durable consumer `budget-service-transaction-created`. On receipt, `OverspendService.HandleTransactionCreated(ctx, userID, now)` lists the user's budgets, groups them by their distinct period start (see above), and for each group recomputes that **current period's** actual spend via the existing synchronous `expenseClient.SumExpensesByCategory` REST call — it does not trust anything about the event payload beyond `user_id`, so a stale or partial event can't corrupt the check. Only the current period is evaluated (an event has no caller-supplied date range the way a report request does); a backdated transaction still correctly contributes to the current period's sum because `SumExpensesByCategory` sums by date range, not by "recently created." A given period's expense-service error is logged and skipped, not fatal to checking the user's budgets in other periods — every budget sharing that period is skipped together this pass, since they share the one call that failed.
